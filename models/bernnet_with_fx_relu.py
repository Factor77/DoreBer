...
from models.Bern_with_relu import BernConv


class BernConvLayer(nn.Module):
    """ A GCN module operated on dependency graphs. """

    def __init__(self, hidden_size, layers, dropout, K=10, rnn_hidden=120, MLP_hidden=32, MLP_dropout=0.5):
        ...
        self.lin1 = Linear(rnn_hidden, MLP_hidden)  
        self.lin2 = Linear(MLP_hidden, rnn_hidden)  
    ...
    def forward(self, adj, input):
        ...
        input = F.dropout(input, p=self.MLP_dropout, training=self.training)
        input = F.relu(self.lin1(input))
        input = F.dropout(input, p=self.MLP_dropout, training=self.training)
        input = F.relu(self.lin2(input)) 
